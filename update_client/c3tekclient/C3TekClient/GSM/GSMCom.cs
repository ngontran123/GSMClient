...
";
        ...
        public string ActivateSIM(string number, string dtmf1, string dtmf2, int delaytimeKH = 20)
        {
            ...
            string blance = ExecuteSingleUSSD("*101#");
            ...
            if (string.IsNullOrEmpty(blance) || blance.Contains("Yeu cau cua quy khach khong the thuc hien") || blance.Contains("chua kich hoat") || blance.Contains("kich hoat") || blance.Contains("Quy khach can") || blance.Contains("goi 900") || blance.Contains("Nghẽn mạng.") || blance.Contains("Kết nối thẻ SIM chưa sẵn sàng") || blance.Contains("ERROR"))
            {
                ...
                string balance = ExecuteSingleUSSD("*101#");
                if (string.IsNullOrEmpty(balance) || balance.Contains("Yeu cau cua quy khach khong the thuc hien") || balance.Contains("Nghẽn mạng.") || balance.Contains("Kết nối thẻ SIM chưa sẵn sàng") || balance.Trim() == "" || balance.Contains("chua kich hoat") || balance.Contains("kich hoat") || balance.Contains("Quy khach can") || balance.Contains("goi 900") || balance.Contains("ERROR"))
                {
                        ...
                        switch (command)
                        {
                            ...
                            case GSMCommand.GET_IMEI:
                                {   
                                    ...
                                    if(!string.IsNullOrEmpty(response) && AppModeSetting.Locale.Equals("km-KH"))
                                    {  
                            ...
                            if (match.Success && !string.IsNullOrEmpty(match.Value))
                            {
                                ...
                                foreach (var msg in msgs)
                                {
                                    try
                                    {
                                        ...
                                        //else if (_isUnicode)
                                        //{
                                        //    content = incomingSmsPdu.GetUserDataTextWithoutHeader();
                                        //}
                                        else
                                        {
                                            content = incomingSmsPdu.UserDataText.Substring(incomingSmsPdu.GetUserDataHeader().Length + 1);
                                        }
